...
class JobsExport extends SpreadsheetExport
{
    private const COLUMNS = [
        'A'  => ['header' => '#',                  'width' => 6],
        'B'  => ['header' => 'Client Name',         'width' => 22],
        'C'  => ['header' => 'Customer ID',         'width' => 13],
    ...
    protected function renderData(Worksheet $sheet): void
    {
        ...
        foreach ($this->jobs as $i => $job) {
            ...
            $sheet->setCellValue('B'  . $row, $job->client?->name ?? '');
            $sheet->setCellValue('C'  . $row, $job->client?->customer_unique_id ?? '');
