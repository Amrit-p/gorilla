...
@else
    <div class="space-y-3">
        @foreach ($jobs as $job)
            ...
            @endphp
            <div
                class="hold-job-card select-none rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition-colors hover:bg-slate-50"
                data-job-id="{{ $job->id }}"
            ...
            >
                ...
                @can('assign-jobs')
                    <div class="mt-3 flex justify-end border-t border-slate-100 pt-2">
                        <x-jobs.reschedule-button :job="$job" class="inline-flex w-full items-center justify-center gap-1.5 rounded-md border border-slate-300 bg-white px-4 py-1.5 text-xs font-medium text-slate-700 transition-colors hover:bg-slate-50">
