...
<div class="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(300px,420px)]">
    <div class="space-y-5 lg:order-1">
            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div class="sm:col-span-2">
                    <x-ui.input
                        ...
                        :value="old('customer_name', $jobModel?->customer_name ?? $selectedClient?->name)"
                        required
                    />
                </div>
                <x-ui.input
                    ...
                    id="job-phone"
                    type="tel"
                    :value="old('phone', $jobModel?->phone ?? $selectedClient?->phone)"
                    required
                />
                <x-ui.input
                    label="Email"
                    ...
                    id="job-email"
                    type="email"
                    :value="old('email', $jobModel?->email ?? $selectedClient?->email)"
                />
            </div>
        </section>
        ...
        <section class="space-y-3 border-t border-slate-100 pt-4">
            ...
            <x-forms.service-types
                ...
                :selected="old('required_services', $jobModel?->required_services ?? $selectedClient?->service_types ?? [])"
            />
        </section>
                    ...
                    <div>
                        ...
                        <select name="zone_id" id="job-zone-id" class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm">
                            ...
                            @foreach ($zones as $zone)
                                <option value="{{ $zone->id }}" @selected((string) old('zone_id', $jobModel?->zone_id ?? $selectedClient?->zone_id) === (string) $zone->id)>{{ $zone->name }}</option>
                            @endforeach
                        </select>
                    </div>
                @endisset
                    ...
                    <div>
                        ...
                        <select name="recurrence_id" id="job-recurrence-id" class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm">
                            ...
                            @foreach ($recurrences as $recurrence)
                                <option value="{{ $recurrence->id }}" @selected((string) old('recurrence_id', $jobModel?->recurrence_id ?? $selectedClient?->recurrence_id) === (string) $recurrence->id)>{{ $recurrence->name }}</option>
                            @endforeach
                        </select>
                    </div>
                @endisset
                ...
                <div>
                    ...
                    <select name="job_type" id="job-job-type" class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm">
                        ...
                        @foreach ($jobTypes as $jobType)
                            <option value="{{ $jobType }}" @selected(old('job_type', $jobModel?->job_type ?? $selectedClient?->job_type) === $jobType)>{{ $jobType }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="weed_spray" id="job-weed-spray" class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm">
                        ...
                        @foreach ($weedSprayOptions as $weedSpray)
                            <option value="{{ $weedSpray }}" @selected(old('weed_spray', $jobModel?->weed_spray ?? $selectedClient?->weed_spray) === $weedSpray)>{{ $weedSpray }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="parking_status" class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm">
                        ...
                        @foreach ($parkingStatuses as $parkingStatus)
                            <option value="{{ $parkingStatus }}" @selected(old('parking_status', $jobModel?->parking_status ?? $selectedClient?->parking_status) === $parkingStatus)>{{ $parkingStatus }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="customer_type" id="job-customer-type" class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm">
                        ...
                        @foreach ($customerTypes as $customerType)
                            <option value="{{ $customerType }}" @selected(old('customer_type', $jobModel?->customer_type ?? $selectedClient?->customer_type) === $customerType)>{{ $customerType }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div>
                    ...
                    <select name="payment_mode" class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm">
                        ...
                        @foreach ($paymentModes as $paymentMode)
                            <option value="{{ $paymentMode }}" @selected(old('payment_mode', $jobModel?->payment_mode ?? $selectedClient?->payment_mode) === $paymentMode)>{{ $paymentMode }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="payment_status" id="job-payment-status" class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm">
                        ...
                        @foreach ($paymentStatuses as $paymentStatus)
                            <option value="{{ $paymentStatus }}" @selected(old('payment_status', $jobModel?->payment_status ?? $selectedClient?->payment_status) === $paymentStatus)>{{ $paymentStatus }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="sm:col-span-2">
                    <x-ui.input label="Charges ($)" name="charges" type="number" min="0" step="0.01" :value="old('charges', $jobModel?->charges ?? $selectedClient?->charges)" />
                </div>
            </div>
        ...
        <section class="space-y-3 border-t border-slate-100 pt-4">
            ...
            <div>
                ...
                <textarea name="site_instructions" id="job-site-instructions" class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" rows="2">{{ old('site_instructions', $jobModel?->site_instructions ?? $selectedClient?->additional_site_instructions) }}</textarea>
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium text-slate-700">Pet warning</label>
                <textarea name="pet_warning" class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" rows="2">{{ old('pet_warning', $jobModel?->pet_warning ?? $selectedClient?->pet_warning) }}</textarea>
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium text-slate-700">Property details</label>
                <textarea name="property_details" rows="2" class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm">{{ old('property_details', $jobModel?->property_details ?? $selectedClient?->property_details) }}</textarea>
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium text-slate-700">Remarks</label>
                <textarea name="special_remarks" class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" rows="2">{{ old('special_remarks', $jobModel?->special_remarks ?? $selectedClient?->special_remarks) }}</textarea>
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium text-slate-700">Internal notes</label>
                <textarea name="notes" rows="2" class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm">{{ old('notes', $jobModel?->notes ?? $selectedClient?->notes) }}</textarea>
            </div>
            <div>
        ...
        @if ($jobModel?->client_id || $selectedClient?->id)
            <input type="hidden" name="client_id" id="job-client-id" value="{{ old('client_id', $jobModel?->client_id ?? $selectedClient?->id) }}">
        @endif
    </div>
    ...
    <div class="lg:order-2 lg:sticky lg:top-4 lg:self-start">
        <x-maps.address-picker
            ...
            :address-value="old('client_address', $jobModel?->client_address ?? $selectedClient?->address)"
            :latitude-value="old('latitude', $jobModel?->latitude ?? $selectedClient?->latitude)"
            :longitude-value="old('longitude', $jobModel?->longitude ?? $selectedClient?->longitude)"
